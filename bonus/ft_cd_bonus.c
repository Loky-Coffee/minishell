...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/14 21:41:35 by aalatzas          #+#    #+#             */
/*   Updated: 2024/05/14 21:41:37 by aalatzas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cd_have_minus(t_ms *ms, char *old_cwd, char *current_cwd)
{
...
static int	ft_cd2(t_cmd *cmd, t_ms *ms, char *old_cwd, char *current_cwd)
{
	if (cmd->tokens[0] && cmd->tokens[0]->next \
	&& ft_strncmp(cmd->tokens[0]->next->str, "-\0", 2) == 0)
		cd_have_minus(ms, old_cwd, current_cwd);
	else if (cmd->tokens[0] && cmd->tokens[0]->next \
